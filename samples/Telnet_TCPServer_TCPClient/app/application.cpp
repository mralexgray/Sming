...
void checkHeap() {
	...
	if (currentHeap == savedHeap) return;
...
void applicationCommand(String commandLine, CommandOutput* commandOutput) {
	...
	commandOutput->printf("Hello from Telnet Example application\r\nYou entered : '%s'\r\nTokenized commandLine is : \r\n", commandLine.c_str());

	Vector<String> commandToken;
	for (int i = 0; i < /*numtoken*/ splitString(commandLine, ' ' , commandToken); i++) 
...
void appheapCommand(String commandLine  ,CommandOutput* commandOutput) {
	...
	if (numToken != 2) commandOutput->printf("Usage appheap on/off/now\r\n");
	...
	else if (commandToken[1] == "now") commandOutput->printf("Heap current free = %d\r\n", system_get_free_heap_size());
	else commandOutput->printf("Usage appheap on/off/now\r\n");
}
...
void connectFail() { debugf("I'm NOT CONNECTED!"); WifiStation.waitConnection(connectOk, 10, connectFail); }
